    ...
    for match in pattern.finditer(line):
        ...
        if tag.startswith('/'):
            ...
            if stack and stack[-1][1] == opening_tag:
                ...
                if 360 <= line_num <= 380:
                    print(f"Line {line_num:4d} closed tag from line {op_line:4d}: {op_raw}")
            else:
                if 360 <= line_num <= 380:
                    print(f"Line {line_num:4d} MISMATCH closing tag: {match.group(0)}")
        else:
            stack.append((line_num, tag, match.group(0)))
            if 360 <= line_num <= 380:
                print(f"Line {line_num:4d} opened: {match.group(0)}")
