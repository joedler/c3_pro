    ...
    for match in pattern.finditer(line):
        ...
        if tag.startswith('/'):
            ...
            if stack and stack[-1][1] == opening_tag:
                ...
                if 298 <= line_num <= 365:
                    print(f"Line {line_num:4d} closed tag from line {op_line:4d}: {op_raw} | Stack Depth: {len(stack)}")
            else:
                if 298 <= line_num <= 365:
                    print(f"Line {line_num:4d} MISMATCH closing tag: {match.group(0)} | Stack Depth: {len(stack)}")
        else:
            stack.append((line_num, tag, match.group(0)))
            if 298 <= line_num <= 365:
                print(f"Line {line_num:4d} opened: {match.group(0)} | Stack Depth: {len(stack)}")
